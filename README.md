...
- **Edge Selective Super Resolution using SinGAN-** Built a function approximator (MLP) over SinGAN to arbitrarily query a low resolution image for real-valued edge co-ordinates to perform superresolution; this was achieved by substituting the SinGAN generators with autoencoders and feeding the encodings to an MLP approximator to predict pixel outputs from input coordinates.
...
- **Smart e-commerce-** Performed time-series forecasting for online retail sales, temporal customer segmentation for targeted marketing, customer network analysis, market-basket analysis and anomaly detection.
